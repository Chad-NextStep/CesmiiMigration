...
/**
 * Fetch a single website (site) page by ID from HubSpot CMS API.
 */
async function fetchSitePage(pageId, token) {
  const url = `${HUBSPOT_API_BASE}/cms/v3/pages/site-pages/${encodeURIComponent(pageId)}`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
module.exports = { fetchLandingPage, fetchSitePage, extractContent, getModuleType };
