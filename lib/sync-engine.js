...
const { fetchLandingPage, fetchSitePage, extractContent } = require('./hubspot-pages');
...
async function syncAll(token, projectRoot) {
  ...
  for (const entry of pageMap.pages) {
    try {
      const fetchFn = entry.pageType === 'site-page' ? fetchSitePage : fetchLandingPage;
      const pageData = await fetchFn(entry.hubspotPageId, token);
