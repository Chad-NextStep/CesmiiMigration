...
/**
 * HubSpot content: loaded in an iframe.
 * See TODO.md — this will be replaced with a proxy approach before launch.
 */
function renderIframeContent(url, title) {
  return `<iframe
      src="${escapeHtml(url)}"
      class="content-frame"
...
function renderShell({ navItems, title, currentPath, contentHtml, contentType }) {
  ...
  const bodyClass = contentType === 'hubspot' ? 'site-body has-iframe' : 'site-body';
...
module.exports = { renderShell, renderIframeContent, renderStaticContent, renderNotFound, renderPlaceholder };
